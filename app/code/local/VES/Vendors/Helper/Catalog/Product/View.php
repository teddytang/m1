 ...
 */
class VES_Vendors_Helper_Catalog_Product_View extends Mage_Core_Helper_Abstract
{
    ...
    public function prepareAndRender($productId, $controller, $params = null)
    {
    	...
    	if($vendorObj->getId() && ($vendorId != $product->getVendorId()) && ($vendorObj->getStatus() == VES_Vendors_Model_Status::STATUS_APPROVED)){
        	$controller->norouteAction();
        	return;
        }
        $this->initProductLayout($product, $controller);
		Mage::dispatchEvent('vendor_product_view_prepare_layout',array('front_action'=>$controller,'vendor'=>$vendorObj));
