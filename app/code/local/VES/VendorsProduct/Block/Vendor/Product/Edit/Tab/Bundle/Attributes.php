 ...
 */
class VES_VendorsProduct_Block_Vendor_Product_Edit_Tab_Bundle_Attributes
    extends VES_VendorsProduct_Block_Vendor_Product_Edit_Tab_Attributes
{
    /**
    ...
    protected function _prepareForm()
    {
        ...
        $sku = $this->getForm()->getElement('vendor_sku');
        if ($sku) {
            $sku->setRenderer(
                $this->getLayout()->createBlock('vendorsproduct/vendor_product_edit_tab_bundle_attributes_extend')
                    ->setDisableChild(false)
            );
        }
        ...
        if ($groupPrice) {
            $groupPrice->setRenderer(
                $this->getLayout()->createBlock('vendorsproduct/vendor_product_edit_tab_price_group')
                    ->setPriceColumnHeader(Mage::helper('bundle')->__('Percent Discount'))
                    ->setPriceValidation('validate-greater-than-zero validate-percents')
            );
        }
        ...
        $mbr = $this->getForm()->getElement('msrp');
        ...
        elseif($this->getCanEditPrice() !== false && $mbr){
            $mbr->setAfterElementHtml(
                '<script type="text/javascript">'
                . "
                function changePriceTypeMap() {
                    if ($('price_type').value == " . Mage_Bundle_Model_Product_Price::PRICE_TYPE_DYNAMIC . ") {
