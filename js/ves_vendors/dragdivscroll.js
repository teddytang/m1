...
var DragDivScroll = function( divId, optionString, funcRef ) /* 15.Mar.14 */
{
 ...
 this.divElem = typeof(divId)=='object'?divId:document.getElementById( divId );
...
var grids = $$('.grid .hor-scroll,.fieldset .hor-scroll');
	if(grids.size()){
		grids.each(function(grid){
			new DragDivScroll( grids.first(), "NOMOUSEWHEEL" );
		});
	}
});